...
class Tello:
    ...
    def __init__(self,
        ...
        # Run obs avoid state UDP reciver on background
        obs_avoid_state_receiver_thread = Thread(target=self.obs_avoid_state_receiver)
        obs_avoid_state_receiver_thread.daemon = True
        obs_avoid_state_receiver_thread.start()


        self.obs_avoid_info = []
        self.obs_avoid_last_received_timestamp = time.time()
...
# ------ OBS AVOID METHODS START ------
    def obs_avoid_state_receiver(self):
        local_ip = ""
        local_port = 9121

        # Create socket for server
        self.obs_avoid_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, 0)
        self.obs_avoid_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.obs_avoid_socket.bind((local_ip, local_port))

        while True:
            try:
                data, address = self.obs_avoid_socket.recvfrom(1024)
                Tello.LOGGER.debug('Data received from {} at obs_avoid_socket'.format(address))

                # decode and store data
                sensor_info = data.decode('utf-8')
                self.obs_avoid_info = list(map(int, sensor_info.split(";")))
                self.obs_avoid_last_received_timestamp = time.time()

            except Exception as e:
                Tello.LOGGER.error(e)
                print("obsac avoid socket error!!")
                break


    def get_obs_avoid_info(self):
        return self.obs_avoid_info

    
    def user_feedback1(self):
        self.obs_avoid_socket.sendto("1".encode('utf-8'), ("192.168.0.152", 2390))  # green blink
        return

    def user_feedback2(self):
        self.obs_avoid_socket.sendto("2".encode('utf-8'), ("192.168.0.152", 2390))  # blue blink
        return

    def start_obs_send(self):
        self.obs_avoid_socket.sendto("5".encode('utf-8'), ("192.168.0.152", 2390))  # blue blink
        return
# ------ OBS AVOID METHODS END ------
